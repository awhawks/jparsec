...
class RenderEclipseMap extends JComponent
{
	...
	protected void offscreenPaint(Graphics2D g)
	{
		...
		{
			Logger.log(Logger.LEVEL.ERROR, "Exception when rendering eclipse. Message was: " + ve.getLocalizedMessage() + ". Trace: " + JPARSECException.getTrace(ve.getStackTrace()));
		}
	}
	 ...
	 */
	public RenderEclipse getRenderEclipse() {
		return rp;
	}
	...
	public final void paintComponent(Graphics g)
	{
		...
		if (render.colorMode == jparsec.graph.chartRendering.Graphics.ANAGLYPH_COLOR_MODE.TRUE_3D_MODE_LEFT_RIGHT) sizew *= 2;
		if (doRedraw)
		{
			doRedraw = false;
