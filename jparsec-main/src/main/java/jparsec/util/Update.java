...
public class Update
{
	...
	public static final String UPDATE_URL_ORBITS_VISUAL_BINARY_STARS = "http://www.stsci.edu/~mperrin/software/gpidata/downloads/config/orb6orbits.txt";
	/**
...
//		file4 += file6; // Not a good idea since some satellites will be twice
		...
		String array[] = DataSet.toStringArray(file5, FileIO.getLineSeparator());
		file5 = "";
		int init = 0;
...
*/
	...
	private static String addAsteroids(String f1, String f2) throws JPARSECException {
		...
		String d1[] = f1.split(sep);
		String d2[] = f2.split(sep);
		Set<String> set = new HashSet<String>();
		//String n1[] = new String[d1.length], n2[] = new String[d2.length];
		StringBuffer buf = new StringBuffer(f1);

		for (int i = 0; i < d1.length; i++) {
			set.add(d1[i].substring(177));
			//n1[i] = d1[i].substring(177);
		}

		for (int i = 0; i < d2.length; i++) {
			String asteroid = d2[i].substring(177);

			if (!set.contains(asteroid)) {
				buf.append(d2[i]);
				buf.append(sep);
			}
			// n2[i] = d2[i].substring(177);
		}
