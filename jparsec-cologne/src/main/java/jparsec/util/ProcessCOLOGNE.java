...
public class ProcessCOLOGNE {
    /**
     * The url used to update COLOGNE catalogue of molecular spectroscopy.
     */
    public static final String COLOGNE_CATALOGUE_URL = "http://www.astro.uni-koeln.de/site/vorhersagen/catalog/";

    public static void main(String[] args) throws Exception {
        Path targetDir = Paths.get(args[0]);
        Files.createDirectories(targetDir);
        String query = COLOGNE_CATALOGUE_URL + "partition_function.html";
        ...
        while (i < lines.length && !lines[i].startsWith("</pre>")) {
    ...
    private static void saveFileFromURL(CloseableHttpClient hc, Path path, String catalogName) throws Exception {
        String query = COLOGNE_CATALOGUE_URL + catalogName;
        HttpGet get = new HttpGet(query);
        CloseableHttpResponse response = hc.execute(get);
        HttpEntity entity = response.getEntity();
