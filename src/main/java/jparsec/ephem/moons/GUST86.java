...
public class GUST86
{
	 ...
	 * the GUST86 theory.
	 * @param GUST86 The output from the GUST86 theory.
	...
	public static MoonOrbitalElement getElements(double GUST86[])
	{
		...
		m.semimajorAxis = GUST86[0] / Constant.AU;
	...
	public static double[] GUST86_theory(double TJJ, int IS, int ICODE)
	{
		...
		ALF = ALF * Constant.DEG_TO_RAD;
		DEL = DEL * Constant.DEG_TO_RAD;
		...
		{
			AN[I] = FQN[I] * T + PHN[I];
			AE[I] = FQE[I] * Constant.DEG_TO_RAD / ANJ * T + PHE[I];
			AI[I] = FQI[I] * Constant.DEG_TO_RAD / ANJ * T + PHI[I];
			AN[I] = Functions.normalizeRadians(AN[I]);
			AE[I] = Functions.normalizeRadians(AE[I]);
			AI[I] = Functions.normalizeRadians(AI[I]);
		}
		...
		EL[1][IS] = Math.pow((RMU[IS] * Constant.SECONDS_PER_DAY * Constant.SECONDS_PER_DAY / (RN * RN)), (1.0 / 3.0));
		RL = Functions.normalizeRadians(RL);
		if (RL < 0.0)
			RL = RL + 2.0 * Math.PI;
		EL[2][IS] = RL;
		EL[3][IS] = RK;
		...
		XU[4][IS] = pXU[4] * Constant.SECONDS_PER_DAY / Constant.AU;
		XU[5][IS] = pXU[5] * Constant.SECONDS_PER_DAY / Constant.AU;
		XU[6][IS] = pXU[6] * Constant.SECONDS_PER_DAY / Constant.AU;
		if (ICODE == 2)
			return new double[]
	...
	private static double[] FK4_TO_FK5(double v[])
	{
		...
		return new double[] { 0.0, V1, V2, V3, V4, V5, V6 };
	}
