...
public class Colors
{
	...
	throws JPARSECException {
		...
		for (i = 0; i < 3; i++)
		{
			...
			results[i] = .001 * ((double) lookup_tbl[tptr_index] * coeff[0] + (double) lookup_tbl[3 + tptr_index] * coeff[1] + (double) lookup_tbl[6 + tptr_index] * coeff[2] + (double) lookup_tbl[9 + tptr_index] * coeff[3]);
		}
