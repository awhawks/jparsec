        ...
        switch (testID) {
            case ATLAS_CHART:
                ...
                for (int i = 0; i < expectedValues.length; i++) {
                    if (!expectedValues[i].trim().equals("-") && !foundValues[i].trim().equals(expectedValues[i].trim()))
                        err += "Output value number " + (i + 1) + " is " + foundValues[i].trim() + " and should be " + expectedValues[i].trim() + FileIO.getLineSeparator();
                }
                break;
            case CONSTELLATION:
                ...
                if (!c.equals(expectedValues[0].trim()))
                    err += "Found constellation name " + c + " and it should be (start with) " + expectedValues[0];
                break;
            case COORDINATE_SYSTEMS:
                ...
                if (!expectedValues[0].trim().equals("-") && !foundValues[0].startsWith(expectedValues[0].trim()))
                    err += "Found position " + foundValues[0] + " and it should be " + expectedValues[0];
                if (!expectedValues[1].trim().equals("-") && !foundValues[1].startsWith(expectedValues[1].trim()))
                    err += "Found position " + foundValues[1] + " and it should be " + expectedValues[1];
                break;
            case EPHEMERIDES:
                ...
                if (dist > sizes)
                    err += "(Accurate) The positions of " + foregroundBody + " and " + backgroundBody + " are separated by " + Functions.formatAngle(dist, 3) + ", which is more than the sum of their sizes and parallaxes (" + Functions.formatAngle(sizes, 3) + ")" + FileIO.getLineSeparator();
                ...
                if (dist > sizes)
                    err += "(Less accurate) The positions of " + foregroundBody + " and " + backgroundBody + " are separated by " + Functions.formatAngle(dist, 3) + ", which is more than the sum of their sizes and parallaxes (" + Functions.formatAngle(sizes, 3) + ")" + FileIO.getLineSeparator();
            ...
            case EPHEMERIDES_NATURAL_SATELLITES:
                ...
                if (!isEclipse) {
                    if (dist > sizes)
                        err += "(Accurate) The positions of " + foregroundBody + " and " + backgroundBody + " are separated by " + Functions.formatAngle(dist, 3) + ", which is more than the sum of their sizes (" + Functions.formatAngle(sizes, 3) + ")" + FileIO.getLineSeparator();
                ...
                if (!isEclipse) {
                    if (dist > sizes)
                        this.out += "(Less accurate) The positions of " + foregroundBody + " and " + backgroundBody + " are separated by " + Functions.formatAngle(dist, 3) + ", which is more than the sum of their sizes (" + Functions.formatAngle(sizes, 3) + ")" + FileIO.getLineSeparator();
                    if (backgroundEphem.status == null || backgroundEphem.status.toLowerCase().indexOf("occulted by") < 0) {
                        if (dist > sizes) {
            ...
            case JULIAN_DAY:
                ...
                if (!expectedValues[0].trim().equals(foundValues[0].trim()))
                    err += "Found Julian day number " + foundValues[0] + " and it should be " + expectedValues[0];
                break;
            case TIME_SCALES:
                ...
                for (int i = 0; i < foundValues.length; i++) {
                    ...
                    if (ms > 0) {
                        ...
                        if (dms > ms)
                            err += "Found Julian day number " + found + " (" + TimeElement.TIME_SCALES_ABBREVIATED[i] + ") and it should be " + expected + " (difference greater than " + ms + " ms)" + FileIO.getLineSeparator();
...
//					if (!expected.equals(found)) err += "Found Julian day number "+found+" ("+TimeElement.TIME_SCALES_ABBREVIATED[i]+") and it should be "+expected+ FileIO.getLineSeparator();
                        ...
                        if (!expected.equals(found))
                            err += "Found Julian day number " + found + " (" + TimeElement.TIME_SCALES_ABBREVIATED[i] + ") and it should be " + expected + FileIO.getLineSeparator();
                    }
                }
                break;
            case MEASURES:
                ...
                if (cal < 0) {
                    ...
                    for (int i = 0; i < outCal.length; i++) {
                        if (outCal[i] != -1) {
                            ...
                            //}
                            foundValues[i] = o;
                            if (!o.equals(expectedValues[i].trim()))
                                err += "Expected output from " + CalendarGenericConversion.CALENDAR_NAMES[outCal[i]] + " calendar was " + expectedValues[i].trim() + ", but " + o + " was found instead." + FileIO.getLineSeparator();
                        }
                    }
                }
                break;
            ...
            case DETAILED_EPHEMERIDES:
                ...
                if (isNaturalSatellite)
                    algor = new EphemerisElement.ALGORITHM[] { EphemerisElement.ALGORITHM.NATURAL_SATELLITE };
                        ...
                        if (algor[j] != EphemerisElement.ALGORITHM.VSOP87_ELP2000ForMoon || eph.targetBody != TARGET.Pluto) {
                            ...
                            if (j > 0) {
                                ...
                                if (expectedValues.length > 6 && eph.ephemType == EphemerisElement.COORDINATES_TYPE.APPARENT) {
                                    ...
                                    for (int k = 0; k < diffs.length; k++) {
                                        String f = fields[k];
                                        if (Math.abs(diffs[k]) > Math.PI)
                                            diffs[k] = Math.abs(diffs[k]) - Constant.TWO_PI;
                                        if (Math.abs(diffs[k]) > acceptableDiffs[k]) {
                                            if (k < 4 || k == 6 || k == 7) {
            ...
            case SIDEREAL_TIME_OBLIQUITY_AND_NUTATION:
                ...
                if (what.equals("siderealtime")) {
                    ...
                    if (!(expectedValues[0].trim()).equals(foundValues[0]))
                        err += "Expected " + expectedValues[0].trim() + ", but found " + foundValues[0] + " instead.";
                    if (!(expectedValues[1].trim()).equals(foundValues[1]))
                        err += "Expected " + expectedValues[1].trim() + ", but found " + foundValues[1] + " instead.";
                    if (!(expectedValues[2].trim()).equals(foundValues[2]))
                        err += "Expected " + expectedValues[2].trim() + ", but found " + foundValues[2] + " instead.";
                }
            ...
            case NATURAL_SATELLITES_AND_JPLDExxx_THEORIES:
                ...
                if (what.startsWith("de")) {
                    for (int i = 0; i < expectedValues.length; i++) {
                        if (!(expectedValues[i].trim()).startsWith(foundValues[i].substring(0, 15)))
                            err += "Expected " + expectedValues[i].trim() + ", but found " + foundValues[i] + " instead." + FileIO.getLineSeparator();
                    }
                } else {
                    for (int i = 0; i < foundValues.length; i++) {
                        if (!(expectedValues[i].trim()).equals(foundValues[i]))
                            err += "Expected " + expectedValues[i].trim() + ", but found " + foundValues[i] + " instead." + FileIO.getLineSeparator();
                    }
                }
                break;
            ...
            case MAIN_EVENTS:
                ...
                for (int i = 0; i < foundValues.length; i++) {
                    if (!(expectedValues[i].trim().toLowerCase()).equals(foundValues[i].trim().toLowerCase()))
                        err += "Expected " + expectedValues[i].trim() + ", but found " + foundValues[i] + " instead." + FileIO.getLineSeparator();
                }
                break;
            case DOUBLE_VARIABLE_STARS:
                ...
                for (int i = 0; i < foundValues.length; i++) {
                    if (!(expectedValues[i].trim().toLowerCase()).equals(foundValues[i].toLowerCase()))
                        err += "Expected " + expectedValues[i].trim() + ", but found " + foundValues[i] + " instead." + FileIO.getLineSeparator();
                }
                break;
            case ECLIPSES:
            ...
            case EPHEMERIDES_OTHER_SOURCES:
                ...
                if (Configuration.JPL_EPHEMERIDES_FILES_EXTERNAL_PATH == null)
